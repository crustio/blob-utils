...
func (cli *Client) PostBlob(ctx context.Context, data []byte) (common.Hash, error) {
	...
	suggestGasPrice, err := cli.client.SuggestGasPrice(ctx)
	if err != nil {
		return common.Hash{}, err
	}
	...
	// estimate blob fee
	blobFee := new(big.Int).Mul(big.NewInt(int64(estimatedBlobGas*escalateMultiplier)), maxFeePerBlobGas)
	...
	legacyTx := types.NewTransaction(uint64(nonce), to, value256.ToBig(), gasLimit, suggestGasPrice, nil)
	...
	blobTx := &types.BlobTx{
		...
		Nonce:      uint64(nonce),
		GasTipCap:  uint256.MustFromBig(suggestGasPrice),
		GasFeeCap:  uint256.MustFromBig(suggestGasPrice),
		Gas:        gasLimit,
		To:         to,
		Value:      value256,
